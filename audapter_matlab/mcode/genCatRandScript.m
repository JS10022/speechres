function [phaseScript, pertDes] = genCatRandScript(phase,  ...
													nBlocks,			...
													trialsPerBlock,		...
													trialTypes,			...		% ==== Empty brackets - "{}" ====
													minDistBetwShifts,	...		% ==== Empty brackets - "{}" ====
													onsetDelays_ms,		...
													numShifts,			...
													interShiftDelays_ms,...
													pitchShifts_cent,	...
...
if ~isempty(fsic(sustPhaseNames, phase))
	assert(isempty(trialTypes));									% ==== Verifies trialTypes is empty====
	trialTypes			= sprintf('{%d-sust}', trialsPerBlock);		% ==== Sets trialTypes to ====
%  	trialTypes
	minDistBetwShifts	= 0;
