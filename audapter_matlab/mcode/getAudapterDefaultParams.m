...
p.downFact	= 4;							% was 4 last year???	=== was 3 ===
% Buffer length (frame size)	(was 96)	=== was 64 ===
p.frameLen	= 512	/ p.downFact;			% must be a valid DMA Buffer size (64 128 256 ..)
p.sr		= 44100 / p.downFact;			% Sampling Rate in Hertz
...
p.rmsThresh		= 0.032 * 10^((getSPLTarg(p.mouthMicDist) - 85) / 20);
...
p.trialLen	= 0;								% 0 corresponds to no length limit (circular)

if ~isempty(findStringInCell(varargin, 'trialLen'))
	p.trialLen = varargin{findStringInCell(varargin, 'trialLen') + 1};
end

p.rampLen	= 0;								% 0 corresponds to no ramp

p.bRMSClip	= 0;								% RMS clipping protection
% p.rmsClipThresh = 1.0;
