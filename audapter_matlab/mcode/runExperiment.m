...
%}
		...
		if ~bIsPertPhase
			...
			gen_pert_pcf(subject.expt_config.OST_MAX_STATE, subject.expt_config.PERT_STATES, 0, 0, 0, pcf_fn);
		...
		for k = 1 : nTrials
			...
			if bIsPertPhase										% Configure perturbation
				...
				if ~(isequal(thisTrial, 'ctrl') || isequal(thisTrial, 'baseline'))
					p.pertAmp = abs(expt.script.(thisphase).(repString).F1Shifts_ratio{k}(1) + 1i ...
								* expt.script.(thisphase).(repString).F2Shifts_ratio{k}(1)) * ones(1, p.pertFieldN);
					p.pertPhi = angle(expt.script.(thisphase).(repString).F1Shifts_ratio{k}(1) + 1i ...
								* expt.script.(thisphase).(repString).F2Shifts_ratio{k}(1)) * ones(1, p.pertFieldN);
