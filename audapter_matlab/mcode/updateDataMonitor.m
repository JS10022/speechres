...
plot(taxis, data.signalOut * handles.dScale);
hold on;
set(gca, 'XLim', [taxis(1);taxis(end)]);
set(gca, 'YLim', [-1, 1]);
xlabel('Time (sec)');
...
%}
	...
	if (~isnan(t1) && ~isnan(t2) && (t1 > 0) && (t2 > 0) && (t2 > t1))
		plot([taxis(t1), taxis(t1)], [ys(1), ys(2)], '-', 'Color', [0.5, 0.5, 0.5], 'LineWidth', 0.5);
		hold on;
		plot([taxis(t2), taxis(t2)], [ys(1), ys(2)], '-', 'Color', [0.5, 0.5, 0.5], 'LineWidth', 0.5);
		hold on;
	...
	for ii = [4, 5]
		...
		if (~isnan(k1) && ~isnan(k2) && (k1 > 0) && (k2 > 0) && (t2 > t1))
			...
			if idx2 <= size(data.fmts, 1)
				plot(taxis1, data.fmts(idx1 : idx2, 1), 'k-', 'LineWidth', 1.5);
				hold on;
			...
			ys = get(gca, 'YLim');
			if ii == 4
				text(xs(1) + 0.025 * range(xs), ys(2) - 0.06 * range(ys), 'Mic. input');
			else
				text(xs(1) + 0.025 * range(xs), ys(2) - 0.06 * range(ys), 'Auditory feedback');
			end
			grid on;
		end
	end
